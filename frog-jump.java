...
// backtrack solution: Time Limit Exceeded
public class Solution {
    Map<Integer, Integer> map = new HashMap<>();
    public boolean canCross(int[] stones) {
        if(stones == null || stones.length < 2 ||
        stones[0] != 0 || stones[1] != 1) return false;
        for(int i = 0; i < stones.length; i++) map.put(stones[i], i);
        return backTrack(stones, 0, 1);
    }
    private boolean backTrack(int[] stones, int curIndex, int jump){
        if(jump < 1) return false;
        if(curIndex >= stones.length) return false;
        
        int target = stones[curIndex] + jump;
        if(!map.containsKey(target)) return false;
        int i = map.get(target);
        if(i == stones.length - 1) return true;
        return backTrack(stones, i, jump - 1) ||
                backTrack(stones, i, jump) ||
                backTrack(stones, i, jump + 1);
    }
}
    ...
    public boolean canCross(int[] stones) {
        ...
        for(int i = 0; i < stones.length; i++){
            for(int step: map.get(stones[i])){
                ...
                // add possible steps to next stone
                if(step - 1 > 0) map.get(target).add(step - 1);
                if(map.containsKey(target + step)) map.get(target).add(step);
                if(map.containsKey(target + step + 1)) map.get(target).add(step + 1);
            }
        }
        return false;
    }
